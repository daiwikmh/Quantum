...
class PlutusBot {
  ...
  private async initiateChatSession(chatId: number): Promise<void> {
    try {
      const ws = new WebSocket("https://plutus-move-agent.onrender.com");
      ...
      ws.on("error", async (error) => {
        ...
        await this.ui.sendErrorMessage(
          chatId,
          "Lost connection to agent. Please try again later."
        );
    ...
    try {
      if (state === "chat") {
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          ...
          await this.ui.sendErrorMessage(
            chatId,
            "Chat session has ended or disconnected."
          );
          this.sessionManager.resetSession(chatId);
        }
  ...
  private async processCallbackData(
    chatId: number,
    data: string
  ): Promise<void> {
    try {
      if (data === "end_chat") {
        this.sessionManager.resetSession(chatId);
        await this.ui.sendMainMenu(
          chatId,
          !!this.sessionManager.getWallet(chatId)
        );
  ...
  private async handleTransactionConfirmation(chatId: number): Promise<void> {
    ...
    if (!state || !market || !amount || !payload) {
      await this.ui.sendErrorMessage(
        chatId,
        "Transaction details missing. Please try again."
      );
      return;
    }
